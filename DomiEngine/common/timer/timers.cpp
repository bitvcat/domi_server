...
void CTimer::addTimerInLoop(CTimerObject* pTimer)
{
	if (!pTimer){
	...
	try
	{
		...
		_timeout.tv_sec = (long)pTimer->m_timeout / 1000;								//秒
		_timeout.tv_usec = (long)(pTimer->m_timeout - _timeout.tv_sec * 1000) * 1000;	//微秒
...
// 添加定时器
uint64 CTimer::addTimer(TimerCallback cbFunc, uint64 _tick, uint32 count)
{
	if (m_shutdown){
		return 0;
	}
	...
	if (!pTimer){
		CLog::error("[CTimer],timer对象构造错误.");
		return 0;
	}

	if (isInLoopThread()){
		addTimerInLoop(pTimer);
	}
	else{
		CCritLocker clLock(m_mutex);
		m_pendingfunc_vec.push_back(std::bind(&CTimer::addTimerInLoop, this, pTimer));

		wakeUp();
	}

	return pTimer->GetSequence();
}
