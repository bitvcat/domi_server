...
public:
	...
	void RejectRead();										// 拒绝读取消息，不在收消息了
	void RejectListen();									// 拒绝监听
	...
	// 虚函数子类实现
	virtual CTcpSession*	ConnectNew(CTcpContext* pContext) = 0;
	virtual CMutex*			ConnectLock() = 0;
	virtual CTcpSession*	ConnectFind(CTcpContext* pContext) = 0;
