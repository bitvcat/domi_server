...
bool CTcpServer::StartServer()
{
	...
	for (uint32 i = 0; i<m_unWorkThreadNum; ++i){	// 初始化work线程
		...
		if (pWorkThread && pWorkThread->initialize()){
...
bool CTcpServer::StopServer()
{
	...
	timeval delay = { 0, 0 };
	// 延时2s关闭监听线程
	if (m_clListenerThread.is_runing()){
		...
		m_clListenerThread.shutdown();	// 等待线程退出
...
void CTcpServer::RejectRead()
{
	CCritLocker contextLocker(this->GetContextMapLock());
	MAP_CONTEXT::iterator it = m_mapContext.begin();
	for (; it != m_mapContext.end();++it){
		if (it->second->m_bufev){
			bufferevent_disable(it->second->m_bufev, EV_READ);
		}
	}
}

void CTcpServer::RejectListen()
{
	if (m_pListener){
...
{
	CTcpSession* pConn = ConnectNew(pContext);
	if (pConn == nullptr){
		pContext->disconnect();
		return;
	}
